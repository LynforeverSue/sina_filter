...
class Spider(CrawlSpider):
    ...
    def parse(self, response):
        ...
        # nickname = "se"
        try:
            nickname = sel.xpath("//title/text()").extract_first()[:-3]

            a = sel.xpath("//div[@class='tip2']/span/text()")
            text =  a.extract_first()[3:-1]
            weibo_num = int(text)
            if (weibo_num >= 200) :
                ...
                for tweet in tweets:
                    ...
                    if others:
                        ...
                        pub_time = others[0]
                        if (u"月" in pub_time or u"前" in pub_time or u"天" in pub_time):
                            pass
                        else :
                            print "old weibo"
                            f = open('old_weibo.txt', 'a')
                            f.write("('http://weibo.com/" + k + "','" + nickname + "',0,NULL,0)," + "\n")
                            f.close()
                            return
                if (total/index >= 50) :
                    #http://weibo.com/haiouhaiou?is_hot=1
                    print "excellent"
                    f = open('f.txt', 'a')
                    f.write("('http://weibo.com/"+k+"','"+nickname+"',0,NULL,0),"+"\n")
                    ...
                    print "small repost num"
                    f = open('small_repost.txt', 'a')
                    f.write("('http://weibo.com/" + k + "','" + nickname + "',0,NULL,0)," + "\n")
                    f.close()
            else:
                print "small weibo num"
                f = open('small_weibo_num.txt', 'a')
                f.write("('http://weibo.com/" + k + "','" + nickname + "',0,NULL,0)," + "\n")
                f.close()
        except:
            traceback.print_exc()
    ...
    def start_requests2(self):
        while len(self.scrawl_ID):
            ...
            for i in range(1,mp+1) :
                url_tweets = "http://weibo.cn/"+ID+"/profile?page="+str(i)
    ...
    def parse2(self, response):
        ...
        # url_next = b[0] + str(j+1)
        #
        # if j < mp:
        #     yield Request(url=url_next , meta={"ID": response.meta["ID"]}, callback=self.parse2)
        # else:
        #     print "---------------------------gg-------------------------------------------------------------"
